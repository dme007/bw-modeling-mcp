...
**Session-holding proxies (`BW_PROXY_CONTEXTID_GUARD`):** Some HTTP proxies in front of BW — the SAP Business Application Studio destination proxy is the known case — strip `Set-Cookie` from responses, keep the SAP `sap-contextid` themselves (per service path) and inject it into every request that carries no `Cookie` header. A stateless request then ends the injected context on the SAP side and every following request fails with HTTP 400 `ICMENOSESSION`; the proxy never recovers by itself. With `BW_PROXY_CONTEXTID_GUARD=true` (or `proxyContextIdGuard: true`) the client (a) sends an explicit empty `Cookie: sap-contextid=` on non-stateful requests while its own jar is empty, so the proxy does not inject — and thereby kill — its stateful context, and (b) heals an `ICMENOSESSION` reply once with a stateful `HEAD` (empty contextid, same path, query string stripped) and retries the request. Stateful requests are untouched, and the guard is a no-op as soon as the jar holds cookies. Off by default.
