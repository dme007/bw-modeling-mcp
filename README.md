...
| `BW_PROXY_CONTEXTID_GUARD` | `true` to work around session-holding proxies (e.g. SAP BAS destination proxy) that inject a dead `sap-contextid` and cause HTTP 400 `ICMENOSESSION`. See ARCHITECTURE.md. | no |
